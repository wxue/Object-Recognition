...
dictionary_kmeans_flag = 1; % set to zero if already got keypoint descriptors (set to 1 if first time running)
k = 500;                    % k in k-means
...
if calculate_kdp_flag == 1
    ...
    for folder = 1:4
        ...
        for n = 1:folder_n
            ...
            if folder == 1
                fprintf('Now processing:\n -- Train/bike/bike_%03d.bmp\n',n);
                IMG = imread(sprintf('Train/bike/bike_%03d.bmp',n));
            end
            if folder == 2
                fprintf('Now processing:\n -- Train/cars/carsgraz_%03d.bmp\n',n);
                IMG = imread(sprintf('Train/cars/carsgraz_%03d.bmp',n));
            end
            if folder == 3
                fprintf('Now processing:\n -- Train/none/bg_graz_%03d.bmp\n',n);
                IMG = imread(sprintf('Train/none/bg_graz_%03d.bmp',n));
            end
            if folder == 4
                fprintf('Now processing:\n -- Train/person/person_%03d.bmp\n',n);
                IMG = imread(sprintf('Train/person/person_%03d.bmp',n));
            end
        ...
        save(sprintf('kpds_%01d.txt', folder),'kpd','-ascii');
        kpd = [];
...
kpd = [];
for n =1:4
    load (sprintf('kpds_%01d.txt',n));
    if n == 1 
        kpd_num_total = length(kpds_1)/128;   % each keypoint descriptor has 128 dimensions    
        kpd_temp = zeros(128, kpd_num_total);
        kpd_temp(:) = kpds_1(:);
    end
    if n == 2 
        kpd_num_total = length(kpds_2)/128;   % each keypoint descriptor has 128 dimensions 
        kpd_temp = zeros(128, kpd_num_total);
        kpd_temp(:) = kpds_2(:);
    end
    if n == 3 
        kpd_num_total = length(kpds_3)/128;   % each keypoint descriptor has 128 dimensions 
        kpd_temp = zeros(128, kpd_num_total);
        kpd_temp(:) = kpds_3(:);
    end
    if n == 4
        kpd_num_total = length(kpds_4)/128;   % each keypoint descriptor has 128 dimensions 
        kpd_temp = zeros(128, kpd_num_total);
        kpd_temp(:) = kpds_4(:);
    end
    kpd = [kpd kpd_temp];
    clear (sprintf('kpds_%01d',n));
end
clear kpds_1; clear kpds_2; clear kpds_3; clear kpds_4;

if dictionary_kmeans_flag == 1
    fprintf('Now processing:\n -- Dictionary Computation(K-means)\n');
    tic;        % TIMER -- start timer
    opts = statset('MaxIter',2000);    % default is 100, it's not enough obviously(maybe 500 times I guess)
