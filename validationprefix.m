...
if calculate_kdp_flag == 1
    ...
    for folder = 1:4
        ...
        for n = 1:folder_n
            ...
            if folder == 1
                n_change = n + 265;
                fprintf('Now processing:\n -- Validation/bike/bike_%03d.bmp\n', n_change);
                IMG = imread(sprintf('Validation/bike/bike_%03d.bmp', n_change));
            end
            if folder == 2
                n_change = n + 220;
                fprintf('Now processing:\n -- Validation/cars/carsgraz_%03d.bmp\n',n_change);
                IMG = imread(sprintf('Validation/cars/carsgraz_%03d.bmp',n_change));
            end
            if folder == 3
                n_change = n + 180;
                fprintf('Now processing:\n -- Validation/none/bg_graz_%03d.bmp\n',n_change);
                IMG = imread(sprintf('Validation/none/bg_graz_%03d.bmp',n_change));
            end
            if folder == 4
                n_change = n + 111;
                fprintf('Now processing:\n -- Validation/person/person_%03d.bmp\n',n_change);
                IMG = imread(sprintf('Validation/person/person_%03d.bmp',n_change));
            end
        ...
        save(sprintf('kpds_validation_%01d.txt', folder),'kpd','-ascii');
        kpd = [];
...
kpd = [];
for n =1:4
    load (sprintf('kpds_validation_%01d.txt',n));
    if n == 1 
        kpd_num_total = length(kpds_validation_1)/128;   % each keypoint descriptor has 128 dimensions    
        kpd_temp = zeros(128, kpd_num_total);
        kpd_temp(:) = kpds_validation_1(:);
    end
    if n == 2 
        kpd_num_total = length(kpds_validation_2)/128;   % each keypoint descriptor has 128 dimensions 
        kpd_temp = zeros(128, kpd_num_total);
        kpd_temp(:) = kpds_validation_2(:);
    end
    if n == 3 
        kpd_num_total = length(kpds_validation_3)/128;   % each keypoint descriptor has 128 dimensions 
        kpd_temp = zeros(128, kpd_num_total);
        kpd_temp(:) = kpds_validation_3(:);
    end
    if n == 4
        kpd_num_total = length(kpds_validation_4)/128;   % each keypoint descriptor has 128 dimensions 
        kpd_temp = zeros(128, kpd_num_total);
        kpd_temp(:) = kpds_validation_4(:);
    end
    kpd = [kpd kpd_temp];
    clear (sprintf('kpds_validation_%01d',n));
end
clear kpds_validation_1; clear kpds_validation_2; clear kpds_validation_3; clear kpds_validation_4;
...
save('imrp_validation.txt','imrp','-ascii');
toc;            % TIMER -- show elapsed time